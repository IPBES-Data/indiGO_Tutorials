...
```{r}
indi_data("biodiversity_habitat_index_hotspots")
```
...
```{r, fig.width=15, fig.height=5}
#indiGO::plot_indiGO("leaf_area_index") JH
```
...
```{r, fig.width=15, fig.height=5}
#indiGO::plot_indiGO("GPP (PgC/yr)") JH ERROR: no dataset yet
```
...
indiGO::plot_indiGO("biodiversity_intactness_index_tropical_hotspots") # JH forest fehlt
...
```{r, fig.width=15, fig.height=5}
#indiGO::plot_indiGO("red_list_index_species used for food and medicine) ") JH: missing
```
...
```{r, fig.width=15, fig.height=5}
#indiGO::plot_indiGO("Red List Index (wild relatives of farmed and domesticated species) ") JH: ERROR does not find name
